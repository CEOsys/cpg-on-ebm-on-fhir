// Author: Gregor Lichtner @glichtner
// Value Sets (/Code Systems) for eligibility criteria categories
Alias: $vs-clinical-findings-sct = http://hl7.org/fhir/ValueSet/clinical-findings
Alias: $vs-allergic-disposition-sct = vs-allergic-disposition-sct
Alias: $vs-episode-of-care-type = vs-episode-of-care-type
Alias: $vs-radiologic-finding-sct = vs-radiologic-finding-sct
Alias: $vs-ventilation-observable-loinc = vs-ventilation-observable-loinc
Alias: $vs-ventilation-observable-sct = vs-ventilation-observable-sct
Alias: $vs-laboratory-observation-loinc = http://loinc.org/vs/top-2000-lab-observations-si
Alias: $vs-procedures-sct = http://hl7.org/fhir/ValueSet/procedure-code

Alias: $cs-kontaktart-de = http://fhir.de/CodeSystem/kontaktart-de
