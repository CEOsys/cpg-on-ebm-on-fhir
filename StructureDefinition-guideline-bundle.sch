...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
  ...
  <sch:pattern>
    ...
    <sch:rule context="f:Bundle">
      ...
      <sch:assert test="not(f:entry/f:request) or (f:type/@value = 'batch') or (f:type/@value = 'transaction') or (f:type/@value = 'history') or (f:type/@value = 'subscription-notification)">entry.request mandatory for batch/transaction/history, allowed for subscription-notification, otherwise prohibited (inherited)</sch:assert>
      <sch:assert test="not(f:entry/f:response) or (f:type/@value = 'batch-response') or (f:type/@value = 'transaction-response') or (f:type/@value = 'history') or (f:type/@value = 'subscription-notification)">entry.response mandatory for batch-response/transaction-response/history, allowed for subscription-notification, otherwise prohibited (inherited)</sch:assert>
      ...
      <sch:assert test="not(f:type/@value = 'document') or exists(f:timestamp/f:value)">A document must have a date (inherited)</sch:assert>
      ...
      <sch:assert test="not(f:type/@value='subscription-notification') or f:entry[1]/f:resource/f:SubscriptionStatus">A subscription-notification must have a SubscriptionStatus as the first resource (inherited)</sch:assert>
      <sch:assert test="(f:type/@value = 'history') and not(f:entry/request/method/@value = 'PATCH')">entry.request.method PATCH not allowed for history (inherited)</sch:assert>
    </sch:rule>
  </sch:pattern>
  <sch:pattern>
